...
export const InputType = z.object({
  analysis_type: z.enum(['single_summary', 'all_summary', 'highlight']),
...
export const OutputType = z.object({
  ...
  stream_results: z.array(z.object({
...
function validateInput(input: AnalysisToolInput): { valid: boolean; error?: string } {
  ...
  // 单篇分析需要证据ID
  if (input.analysis_type === 'single_summary' || input.analysis_type === 'highlight') {
    if (isEmpty(input.evidence_id)) {
      return { valid: false, error: '单篇分析需要提供证据ID' };
    }
    if (!isValidEvidenceId(input.evidence_id!)) {
      return { valid: false, error: '证据ID格式无效' };
    }
  }

  // 批量分析需要问题ID
  if (input.analysis_type === 'all_summary') {
    if (isEmpty(input.question_id)) {
      return { valid: false, error: '批量分析需要提供问题ID' };
    }
    if (!isValidQuestionId(input.question_id!)) {
 ...
 * 执行单篇证据总结
 */
async function executeSingleSummary(
  client: any, 
  evidenceId: string
...
export async function tool(input: z.infer<typeof InputType>): Promise<z.infer<typeof OutputType>> {
  try {
    ...
    switch (input.analysis_type) {
      case 'single_summary': {
        const summaryResult = await executeSingleSummary(client, input.evidence_id!);
      ...
      case 'highlight': {
        const highlightResult = await executeHighlight(client, input.evidence_id!);
        result.highlights = highlightResult.highlights;
        result.evidence_count = 1;
        
        // 生成高亮摘要
        const blockTypeCounts = highlightResult.highlights.reduce((acc, h) => {
    ...
    const message = `${input.analysis_type === 'single_summary' ? '单篇证据' : 
                     input.analysis_type === 'all_summary' ? '批量证据' : '证据高亮'}分析完成`;
