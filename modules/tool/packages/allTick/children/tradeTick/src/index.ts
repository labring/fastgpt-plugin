...
// Single product latest trade price data schema
const TickItemType = z.object({
  code: z.string().describe('Product code'),
  seq: z.string().describe('Sequence number'),
  tick_time: z.string().describe('Timestamp'),
  price: z.string().describe('Trade price'),
  volume: z.string().describe('Trade volume'),
  turnover: z.string().describe('Trade turnover'),
  trade_direction: z.number().describe('Trade direction: 0=default, 1=Buy, 2=SELL')
});
...
function getApiEndpoint(isStock: boolean): string {
  if (isStock) {
    return 'https://quote.alltick.io/quote-stock-b-api/trade-tick';
  } else {
    return 'https://quote.alltick.io/quote-b-api/trade-tick';
  }
}
...
export async function tool(params: z.infer<typeof InputType>): Promise<z.infer<typeof OutputType>> {
  try {
    ...
    if (!validatedResponse.data || !validatedResponse.data.tick_list) {
      return Promise.reject(
        new Error(
          'Failed to retrieve latest trade price data, please check if the product code is correct'
        )
      );
    }
