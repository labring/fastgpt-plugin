...
| 字段 | 说明 |
| --- | --- |
| 部署方式 | 选择 `SoMark API` 或 `私有化部署`。 |
| API Key | 使用 SoMark 官方 API 时填写；私有化部署无需填写。 |
| Base URL | 私有化部署时填写服务地址；使用 SoMark 官方 API 时留空即可。 |

使用 SoMark 官方 API 时，工具会默认请求 `https://somark.tech/api/v1`。私有化部署时必须填写可访问的 SoMark 服务地址。
...
- 使用 SoMark 官方 API 时必须填写 API Key。
- 使用私有化部署时不会发送 `api_key` 字段。
