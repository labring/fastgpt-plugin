...
1. `fastgpt-plugin`
   本仓库，Monorepo 架构，包含 Plugin Server、SDK、CLI、调试监视器和基础设施代码。

2. `fastgpt-official-plugins`
   官方系统插件仓库，收录由 FastGPT 官方维护或审核通过的插件。官方插件需要经过更严格的代码、功能、安全和上架测试。

3. `fastgpt-community-plugins`
   社区插件仓库，用于承载社区提交的第三方插件。社区插件上架 Marketplace 时会保留来源和风险提示。

4. `fastgpt-business-plugins`
   私有插件仓库，用于闭源插件、客户定制插件和商业交付插件。

本仓库只提供开发、构建、检查、打包和服务端运行能力。具体插件源码通常放在 official、community 或 business 插件仓库中。
...
| 文件能力 | 是否需要上传文件，上传时使用 `ctx.invoke.uploadFile()`。 |
| 流式输出 | 是否需要向用户展示中间进度，流式输出使用 `ctx.streamResponse()`。 |
| 测试样例 | 至少包含成功路径、参数错误、鉴权失败和上游失败。 |

缺失信息可以使用合理默认值继续推进时，应在提交说明中记录假设；影响插件 ID、鉴权方式、计费或上架安全性的缺失信息需要先确认。
...
在本仓库内开发或维护 SDK/CLI 时，也可以参考本地 Skill：
...
- `pnpm`，本仓库使用 `pnpm@10.33.2`。
- Git。
- GitHub CLI `gh`，用于 fork、创建仓库和提交 PR。

开发社区插件时，先 fork 并 clone 社区仓库：
...
在本仓库调试 CLI 或 SDK 时，先安装依赖并构建 CLI/SDK：
...
- 需要调用宿主文件上传能力时，使用 `ctx.invoke.uploadFile()`，并优先保留返回的 `err`。
- 需要向用户展示进度时，使用 `ctx.streamResponse()`。
...
社区仓库的 `packages/` 下通常引用第三方插件仓库。开发完成后，在插件目录创建并推送独立 GitHub 仓库：
...
### 文件名为什么是 `how-to-devlop-plugin.md`？

当前文件名保留历史拼写，避免破坏已有链接。后续可以在统一更新文档引用时迁移到 `how-to-develop-plugin.md`，并保留跳转或兼容入口。

## 参考

- [FastGPT 插件系统设计文档](./design.zh.md)
- [FastGPT Plugin 架构文档](./architecture.zh.md)
- [FastGPT Plugin SDK Factory](../../sdk/factory/README.md)
- [系统工具开发 Skill](../../sdk/factory/skills/fastgpt-system-tool-development/SKILL.md)
- [local-pool 运行时设计](./process-pool-design.zh.md)
