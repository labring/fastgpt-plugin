...
describe('PluginServiceManager - 多插件管理', () => {
  ...
  describe('服务注册与注销', () => {
    ...
    it('应该拒绝重复注册同名服务', async () => {
      ...
      await expect(
        manager.registerService('weather', pluginPath, weatherConfig)
  ...
  describe('invoke 路由', () => {
    ...
    it('应该正确路由到指定服务', async () => {
      ...
      const translateResult = await manager.invoke('translate', 'translate', { text: 'hello', to: 'zh' });
    ...
    it('应该在调用不存在的服务时抛出错误', async () => {
      await expect(
        manager.invoke('nonexistent', 'method', {})
      ).rejects.toThrow('Service does not exist');
    });
  ...
  describe('事件系统', () => {
    it('应该触发 serviceRegistered 事件', async () => {
      ...
      expect(handler).toHaveBeenCalledWith(
        expect.objectContaining({ serviceName: 'weather' })
      );
    });
  ...
  describe('动态配置更新', () => {
    ...
    it('应该在更新不存在的服务配置时抛出错误', async () => {
      await expect(
        manager.updateServiceConfig('nonexistent', weatherConfig)
      ).rejects.toThrow('Service does not exist');
    });
  });
