...
import { type PluginManagerPort } from '@fastgpt-plugin/domain/ports/plugin-manager.port';
import type { SystemModeType } from '@fastgpt-plugin/domain/value-objects/system-mode.enum';
import type { PluginType, PluginTypeType } from '@fastgpt-plugin/domain/entities/plugin.entity';
import { env } from '@/env';
import type { PluginManagerConfig, PluginRecord } from './type';
 ...
 *
 */
export class PluginManager extends EventEmitter implements PluginManagerPort<> {
  ...
  protected constructor() {
    ...
    this.config = {
      ...
      maxTotalPods: env.MAX_TOTAL_PODS,
  ...
  async register(id: string, registration: PluginRegistration): Promise<void> {
    ...
    service.on(
      'podLog',
      ({ podId, level, message }: { podId: string; level: 'debug' | 'error'; message: string }) => {
