...
import { signCallbackToken, verifyCallbackToken } from '../jwt/index';
import type {
  ServiceConfig,
  ServiceMetrics,
  InvokeOptions,
...
export class PluginService extends EventEmitter {
  ...
  /** 反向调用注册表：traceId → callbackHandler */
  private callbackRegistry = new Map<
    string,
  ...
  async invoke(method: string, params: any, options?: InvokeOptions): Promise<any> {
    ...
    if (options?.callbackHandler) {
      traceId = randomUUID();
      const callbackToken = await signCallbackToken(traceId, this.serviceName);
      this.callbackRegistry.set(traceId, options.callbackHandler);
      invokeParams = { ...params, callbackToken };
    }
    ...
    if (pod) {
      try {
        const result = await pod.invoke(method, invokeParams, options);
        ...
        if (traceId) this.callbackRegistry.delete(traceId);
      }
    }

    // 没有可用 Pod，排队等待
    return this.queueAndWait(method, invokeParams, options, startTime, traceId);
  }

  /**
   * 处理子进程反向调用（验证 JWT，查 registry，执行 handler）
   */
  private async handleHostCallback(
    token: string,
    method: string,
    params: unknown
  ): Promise<unknown> {
    const { traceId } = await verifyCallbackToken(token);
    const handler = this.callbackRegistry.get(traceId);
    if (!handler) throw new Error('Callback context expired or not found');
  ...
  private async createPod(): Promise<PluginPod> {
    ...
    const pod = new PluginPod(podId, {
      ...
      onHostCallback: (token, method, params) => this.handleHostCallback(token, method, params)
    });
